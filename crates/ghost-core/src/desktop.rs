...
impl DesktopSession {
    ...
    pub fn click(&self, hwnd: isize, x: i32, y: i32) -> Result<(), CoreError> {
        self.exec(move || crate::input::postmessage::click(as_hwnd(hwnd), (x, y)))?
    }

    pub fn right_click(&self, hwnd: isize, x: i32, y: i32) -> Result<(), CoreError> {
        self.exec(move || crate::input::postmessage::right_click(as_hwnd(hwnd), (x, y)))?
    }

    pub fn double_click(&self, hwnd: isize, x: i32, y: i32) -> Result<(), CoreError> {
        self.exec(move || crate::input::postmessage::double_click(as_hwnd(hwnd), (x, y)))?
    }

    pub fn hover(&self, hwnd: isize, x: i32, y: i32) -> Result<(), CoreError> {
        self.exec(move || crate::input::postmessage::hover(as_hwnd(hwnd), (x, y)))?
    }
    ...
    pub fn scroll(
        ...
        self.exec(move || {
            crate::input::postmessage::scroll(as_hwnd(hwnd), (x, y), notches, horizontal)
        })?
    }

    /// Type into a window on this desktop.
    pub fn type_text(&self, hwnd: isize, text: &str) -> Result<(), CoreError> {
        let t = text.to_string();
        self.exec(move || crate::input::postmessage::type_text(as_hwnd(hwnd), &t))?
    }

    pub fn press(&self, hwnd: isize, key: &str) -> Result<(), CoreError> {
        let k = key.to_string();
        self.exec(move || match crate::input::keyboard::name_to_vk(&k) {
            Some(vk) => crate::input::postmessage::press_key(as_hwnd(hwnd), vk.0),
    ...
    /// messages rather than faked modifier keys. See `input::shortcut`.
    pub fn shortcut(&self, hwnd: isize, name: &str) -> Result<(), CoreError> {
        let n = name.to_string();
        self.exec(move || crate::input::shortcut::apply(as_hwnd(hwnd), &n))?
    }
    ...
    /// PNG of one window on this desktop.
    pub fn capture(&self, hwnd: isize, client_only: bool) -> Result<Vec<u8>, CoreError> {
        self.exec(move || {
            crate::capture::capture_window(HWND(hwnd as *mut core::ffi::c_void), client_only)
        })?
    }
